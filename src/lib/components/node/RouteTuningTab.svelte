<script lang="ts">
	...
	// 读写走 /ui/…/route-tuning 专用端点：字段级合并在服务端做，前端不再
	// 读改写整棵 base_template / session spec（并发编辑不会互相覆盖）。
	...
	async function load() {
		...
		err = '';
		try {
			const v = await api.routeTuning(nodeId);
			coldPotatoMed = v.cold_potato_med;
			rules = v.route_local_pref.map((r) => ({ prefix: r.prefix, local_pref: r.local_pref }));
			sessRows = v.sessions.map((s) => ({
				id: s.id,
				name: s.name,
	...
	async function saveNode() {
		savingNode = true;
		try {
			// 服务端只合并这两个 key，base_template 其余部分不受影响。
			const v = await api.saveRouteTuning(nodeId, {
				cold_potato_med: coldPotatoMed === '' ? 50 : Number(coldPotatoMed),
				route_local_pref: rules
	...
	async function saveSession(row: SessRow) {
		savingSid = row.id;
		try {
			// 服务端在 session spec 上做单字段合并。
			await api.setLinkLatency(row.id, row.lat === '' ? null : Number(row.lat));
